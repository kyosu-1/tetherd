...
| Developer ID 署名・公証 | cask の `postflight_steps` が quarantine を外す（付いたままのバイナリは **Gatekeeper に止められる** — v1.0 実測）（brew でも属性は必ず付く。§8） | Developer ID 取得後 |
...
- **formula ではなく cask。** 3 バイナリを prefix に置くだけなのは変わらない（v0.4 実測: cask DSL の `binary` スタンザ 3 本）。cask にした理由は署名していないこと: Homebrew は cask のダウンロードを必ず quarantine するので、`xattr -dr com.apple.quarantine` を外す必要があり、formula にはその口が無い。**quarantine が残ったバイナリは実行できない**（v1.0 実測: Caskroom のバイナリに属性を付け直すと macOS がモーダルで止め、プロセスは起動しない）ので、これは整頓でなく必須の処理。**使うスタンザは `postflight_steps`。** 古い `postflight` は非推奨で、Homebrew の非推奨ヘルパは既定で `disable_for_developers: true` なので **`HOMEBREW_DEVELOPER` を設定している人には警告でなく `MethodDeprecatedError` になり `brew install` が失敗する**（v1.0 実測: 公開済み v0.4.1 の cask は exit 1、同じコマンドを env 無しで打つと exit 0 で警告のみ — この非対称性のため v0.4 の実機検証をすり抜けた）。代償として `postflight_steps` は **Homebrew 5.1.14 以降**を要求し、それより古いと `NoMethodError`。cask の `depends_on` に Homebrew のバージョンを要求するキーは無いので宣言では防げず、文書に書くしかない（`brew` は既定で 24 時間ごとに自動更新するので実際の露出は小さい）。GoReleaser の cask テンプレートは `main` でも `postflight` を吐くため、`hooks` は 4 つすべて使わず `custom_block` に生の `postflight_steps` を注入する。`service` ブロックは使わない（launchd の登録は helper 自身が行う）。cask の `uninstall launchctl:` / `delete:` は `brew uninstall` が daemon を止めるための保険で、アンインストールの本体ではない（下記）
...
さらに: `doctor` は **14 行すべて `✓` で exit 0**、`tetherd run -- aws sts get-caller-identity` が子プロセスにタスクロールを渡し（helper ログに `pf enabled → applied → cleared` の全ライフサイクル、終了後 `/etc/resolver` は空）、`sudo tetherd-helper install` は冪等で daemon を入れ替える、`brew uninstall --cask` は daemon を落とす。quarantine は `postflight` で除去されている（v1.0 で `postflight_steps` に置き換えた）。
